...
I have tried to keep the scenario as real and practical as possible which initiates from the Delivery phase of the Cyber Kill Chain(if you don't know what that is [here](https://www.lockheedmartin.com/en-us/capabilities/cyber/cyber-kill-chain.html) is a link to read up on it) and this is not one of those impractical posts showing the results of a VirusTotal scan against a freshly-coded simple reverse shell to judge whether it'll pass as the payload for an actual red-teaming engagement and if so terming it as a bypass.
