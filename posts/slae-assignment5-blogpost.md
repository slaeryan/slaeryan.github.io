...
### dup2 syscall
This step is same as the one we evaluated in the Bind TCP payload(i.e. duplicating into our accepted connection socket which we got back from the previous step the STDIN/0, STDOUT/1, and STDERR/2 file descriptors(fd) to make the connection interactive for the attackers).
### connect syscall
